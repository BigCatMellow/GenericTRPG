...
export function snapPointToGrid(point, board) {
  // Snap to grid square centers (0.5, 1.5, 2.5, ...) not line intersections
  return {
    x: clamp(Math.floor(point.x) + 0.5, 0.5, board.widthInches - 0.5),
    y: clamp(Math.floor(point.y) + 0.5, 0.5, board.heightInches - 0.5)
  };
}
